...
describe("Test ADR-36 Amino Sign Doc", () => {
    ...
    it("Verify ADR-36 Amino sign doc", () => {
        ...
        expect(amino_1.verifyADR36AminoSignDoc("osmo", signDoc, pubKey.toBytes(), signature.slice().map((b) => (Math.random() > 0.5 ? 0 : b)))).toBe(false);
    ...
    it("Verify ADR-36 Amino sign doc with ethsecp256k1", () => {
        const privKey = crypto_1.PrivKeySecp256k1.generateRandomKey();
        const pubKey = privKey.getPubKey();
        const signer = new bech32_1.Bech32Address(pubKey.getEthAddress()).toBech32("eth");
        const signDoc = amino_1.makeADR36AminoSignDoc(signer, new Uint8Array([1, 2, 3]));
        const msg = signing_1.serializeSignDoc(signDoc);
        const signature = privKey.signDigest32(crypto_1.Hash.keccak256(msg));
        expect(amino_1.verifyADR36AminoSignDoc("eth", signDoc, pubKey.toBytes(), signature, "ethsecp256k1")).toBe(true);
        expect(amino_1.verifyADR36Amino("eth", signer, new Uint8Array([1, 2, 3]), pubKey.toBytes(), signature, "ethsecp256k1")).toBe(true);
        expect(() => amino_1.verifyADR36Amino("eth", 
        // Unmatched signer
        new bech32_1.Bech32Address(pubKey.getAddress()).toBech32("eth"), new Uint8Array([1, 2, 3]), pubKey.toBytes(), signature, "ethsecp256k1")).toThrow();
        expect(() => amino_1.verifyADR36Amino("eth", 
        // Invalid signer
        "invalid1ymk637a7wljvt4w7q9lnrw95mg9sr37yatxd9h", new Uint8Array([1, 2, 3]), pubKey.toBytes(), signature, "ethsecp256k1")).toThrow();
        expect(amino_1.verifyADR36AminoSignDoc("eth", signDoc, pubKey.toBytes(), signature.slice().map((b) => (Math.random() > 0.5 ? 0 : b)), "ethsecp256k1")).toBe(false);
        expect(amino_1.verifyADR36Amino("eth", signer, new Uint8Array([1, 2]), pubKey.toBytes(), signature, "ethsecp256k1")).toBe(false);
    });
});
